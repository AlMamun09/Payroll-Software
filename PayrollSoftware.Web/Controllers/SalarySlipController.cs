...
_payrollRepository = payrollRepository;
   ...
   .AsNoTracking()
    .Where(e => employeeIds.Contains(e.EmployeeId))
       .ToDictionaryAsync(e => e.EmployeeId, e => new
         {
       e.EmployeeCode,
  e.FullName
      });
  ...
  // Get payroll details
   ...
   .AsNoTracking()
     .Where(p => payrollIds.Contains(p.PayrollId))
          .ToDictionaryAsync(p => p.PayrollId, p => new
     {
        p.PayPeriodStart,
         p.PayPeriodEnd,
    p.PaymentStatus
    ...
    var dtos = salarySlips.Select(s => new
  {
                    s.SalarySlipId,
       s.PayrollId,
   s.EmployeeId,
    EmployeeCode = employees.TryGetValue(s.EmployeeId, out var emp) ? emp.EmployeeCode : "N/A",
        EmployeeName = employees.TryGetValue(s.EmployeeId, out var emp2) ? emp2.FullName : "Unknown",
        s.Month,
   s.Year,
     MonthYear = $"{GetMonthName(s.Month)} {s.Year}",
   PayPeriodStart = payrolls.TryGetValue(s.PayrollId, out var p) ? p.PayPeriodStart : DateTime.MinValue,
        PayPeriodEnd = payrolls.TryGetValue(s.PayrollId, out var p2) ? p2.PayPeriodEnd : DateTime.MinValue,
     s.GrossEarnings,
           s.TotalDeductions,
        s.NetPay,
         s.GeneratedDate,
         PaymentStatus = payrolls.TryGetValue(s.PayrollId, out var p3) ? p3.PaymentStatus : "Unknown"
  }).ToList();
            ...
            catch (Exception ex)
 {
    ...
    public async Task<IActionResult> View(Guid id)
  {
     ...
     return RedirectToAction(nameof(Index));
      ...
      return RedirectToAction("ViewSalarySlip", "Payroll", new { id = salarySlip.PayrollId });
            }
catch (Exception ex)
            {
