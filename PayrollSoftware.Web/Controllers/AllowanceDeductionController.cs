  ...
  public class AllowanceDeductionController : Controller
  {
   ...
   ApplicationDbContext context)
    ...
    public async Task<IActionResult> GetAllowanceDeductionsJson()
    {
      try
      {
        var allowanceDeductions = await _allowanceDeductionRepository.GetAllAllowanceDeductionsAsync();
        var dtos = allowanceDeductions.Select(a => new
        {
       ...
       ? _context.Employees.FirstOrDefault(e => e.EmployeeId == a.EmployeeId)?.FullName
       : "N/A"
        }).ToList();
    ...
    public async Task<IActionResult> Create()
    {
      ...
      return View("Create", new AllowanceDeductionDto
      {
        IsActive = true,
    ...
    public async Task<IActionResult> Create([FromForm] AllowanceDeductionDto dto)
    {
      try
      {
        ...
        return Json(new { success = true, message = "Allowance/Deduction created successfully." });
      }
      ...
      {
        return StatusCode(500, new { success = false, message = $"Error creating allowance/deduction: {ex.Message}" });
      }
    }
    ...
    public async Task<IActionResult> Edit([FromForm] AllowanceDeductionDto dto)
    {
      try
      {
        ...
        return Json(new { success = true, message = "Allowance/Deduction updated successfully." });
      }
      catch (ArgumentException ex)
      {
        return BadRequest(new { success = false, message = ex.Message });
      ...
      {
        return StatusCode(500, new { success = false, message = $"Error updating allowance/deduction: {ex.Message}" });
      }
    }
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
      try
      {
        ...
        if (entity == null)
        {
          return NotFound(new { success = false, message = "Allowance/Deduction not found." });
        }

        await _allowanceDeductionRepository.DeleteAllowanceDeductionAsync(id);
        return Json(new { success = true, message = "Allowance/Deduction deleted successfully." });
      }
      catch (InvalidOperationException ex)
      {
        return BadRequest(new { success = false, message = ex.Message });
      ...
      {
        return StatusCode(500, new { success = false, message = $"Error deleting allowance/deduction: {ex.Message}" });
      }
    }
